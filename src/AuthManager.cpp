...
void AuthManager::reset() {
    ...
    if (!sessionMutex) {
        sessionMutex = xSemaphoreCreateMutex();
        if (!sessionMutex) {
            Logger::error("Failed to create session mutex during reset");
            return;
        }
    }
    ...
    bool success = setCredentials("admin", "admin");
    if (success) {
        Logger::info("Authentication reset successful");
    } else {
        Logger::error("Failed to reset authentication credentials");
    }
}
